  ...
  try {
    const statsSql = `
      SELECT
        ...
        COALESCE(AVG(expense_other), 0)::int     AS exp_other_avg,
        COALESCE(AVG(
          CASE WHEN monthly_salary > 0 THEN
            (monthly_salary - (expense_housing + expense_food + expense_transport
              + expense_leisure + expense_shopping + expense_other))::numeric
            / monthly_salary
          END
    ...
    const total_exp_avg =
      ...
      stats.exp_leisure_avg + stats.exp_shopping_avg + stats.exp_other_avg;
    ...
    const expense_breakdown = [
      ...
      { key: 'other',     label: '기타',   avg: stats.exp_other_avg },
    ].map(x => ({
      ...x,
    ...
    if (me) {
      ...
      const myExp = num(me.expense_housing) + num(me.expense_food) + num(me.expense_transport)
                  + num(me.expense_leisure) + num(me.expense_shopping) + num(me.expense_other);
