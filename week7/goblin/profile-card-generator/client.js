...
function ImageCell({ data, filename }) {
  if (!data) {
    return <div style={{aspectRatio: '1200/630'}} className="w-full bg-neutral-800 rounded flex items-center justify-center text-neutral-500 text-xs">1200×630</div>;
  }
    ...
    <div className="bg-neutral-900 rounded-lg border border-neutral-800 p-5 flex flex-col gap-4">
      ...
      {status === 'loading' && (
        <div className="text-sm text-amber-400 flex items-center gap-2">
          ...
          1200×630 생성 중 (약 15~25초)
        </div>
      )}
      {status === 'error' && <div className="text-sm text-red-400">에러: {error}</div>}

      <ImageCell data={image} filename={`${tone.key}_1200x630.png`} />
    </div>
  );
}
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      <header className="mb-10">
        <h1 className="text-3xl font-bold">프로필 카드 톤 생성기</h1>
        <p className="text-neutral-400 mt-2">5개 톤 × 1200×630 가로 카드 = 5장. OG / 블로그 / LinkedIn 표준. 좌측 비주얼 + 우측 텍스트 여백 구도.</p>
        <p className="text-xs text-neutral-500 mt-2">모델 gpt-image-1 (medium) · 톤당 약 $0.06 · 텍스트 합성 없음, 순수 비주얼 배경.</p>
